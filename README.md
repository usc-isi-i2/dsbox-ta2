...
```
docker run --name isi-dsbox -it /bin/bash
```
...
The fitted pipelines generated by the DSBox system is store in the directory defined by the `executables_root` in the `search_config.json` file. The name of DSBox fitted pipelines have the form `<UUID>.json`. To run a fitted pipeline, say `0ecd7d2e-4620-430a-881e-b93ff0cfb692.json`, on the `38_sick` problem, do:
...
bound to a single primitive, whereas a DSBox template node can be bound to multiple
...
The Python dictionary below is an example of a DSBox template step specification. The name
of the step is `model_step`, and this step can instantiated with three posible primitives:
