...
def split_dataset(dataset, problem, problem_loc=None, *, random_state=42, test_size=0.2):
    ...
    if task_type == TaskType.CLASSIFICATION:
        ...
        if not task_type == TaskType.REGRESSION:
            print("\n*\nIt might be a good idea to log this...")
    ...
    print("\n*\nToo bad we're not logging this...")
    print(meta)
...
class Controller:
    ...
    def write_training_results(self):
        ...
        # TODO: does this 'return None' do anything?
        return None
    ...
    def search_template(self, template: DSBoxTemplate, candidate: typing.Dict=None,
        ...
        if candidate is None:
            self._logger.error("[ERROR] Candidate is 'None'", exc_info=True)
            return Status.PROBLEM_NOT_IMPLEMENT
        else:
            self._logger.info("******************\n[INFO] Writing results")
            pprint.pprint(candidate.data)
            self._logger.info("Candidate: {}, \nValue: {}".format(candidate, value))
            ...
            self._logger.info("******************\n[INFO] Saving training results in {}".format(save_location))
            try:
    ...
    def train(self) -> Status:
        ...
        for idx in self.select_next_template(max_iter=5):
            ...
            self._logger.info(STYLE+"[INFO] Template {}:{} Selected. UCT:{}".format(
            ...
            self._logger.info(STYLE+"[INFO] cache size: {}, candidates: {}".format(len(cache), len(candidate_cache)))
            # break
    ...
    def test(self) -> Status:
        ...
        try:
            ...
            self._logger.error("[Warning] Can't find the prediction class name, will use default name.")
