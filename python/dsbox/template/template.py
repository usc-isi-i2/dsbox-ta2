...
from d3m.metadata.pipeline import Pipeline, StepBase, \
    PrimitiveStep, PlaceholderStep, SubpipelineStep, Resolver
# from d3m.metadata.pipeline import PIPELINE_SCHEMA_VALIDATOR
...
class TemplatePipeline(Pipeline):
            ...
            if directive == HyperparamDirective.RANDOM:
                ...
                for key, value in hyperparams_class.sample().items():
                    if (value is None
                        ...
                        or issubclass(type(value), str)):
                        argument_type = metadata_base.ArgumentType.DATA
                    else:
                        raise ValueError(
    ...
    def from_json(cls: typing.Type[TP], pipeline_description: typing.Dict, *,
        ...
        # TODO: need to be updated with latest d3m release
