...
class Client(object):
    ...
    def main(self, argv):
        ...
        parser = argparse.ArgumentParser(description='TA3 Test client')
        parser.add_argument('--docker', action='store_true',
                            help='The dataset path deepnds on if server is running on docker or not')
        parser.add_argument('--port', help='TA2 server port', default=PORT)
    ...
    def fitSolution(self, stub, solution_id, dataset_info):
        ...
        _logger.info("    With dataset uri " + dataset_info.dataset_uri)
